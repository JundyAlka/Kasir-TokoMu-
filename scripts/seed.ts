...
function monthEndMonthsAgo(monthsAgo: number) {
  const nextMonthStart = monthStartMonthsAgo(monthsAgo - 1);
  nextMonthStart.setMilliseconds(-1);
...
async function seedTokomuExtensions(
  userId: string,
  productList: Array<SeedProduct & { dbId: string }>
) {
  const timestamp = new Date().toISOString();
  const productBySeedId = new Map(productList.map((product) => [product.id, product]));
  ...
  const investorRows = [
    {
      ...
      name: "H. Ahmad Fauzan",
      whatsapp: "081226789001",
      address: "Dusun Krajan, Grabag",
      notes: "Investor uang Rp5.000.000 dengan porsi bagi hasil 10%.",
      isActive: 1,
      createdAt: timestamp,
      updatedAt: timestamp,
    },
    {
      ...
      workspaceOwnerId: userId,
      name: "Bu Siti Aminah",
      whatsapp: "081327654112",
      address: "Grabag, Purworejo",
      notes: "Barang titip jual mi instan, 50 dus @ Rp80.000, bagi hasil 15% per unit.",
      isActive: 1,
      createdAt: timestamp,
      updatedAt: timestamp,
    },
    {
      ...
      whatsapp: "081334455209",
      address: "Secang, Magelang",
      notes: "Investor uang Rp3.000.000 untuk penguatan stok sembako.",
      isActive: 1,
      createdAt: timestamp,
      updatedAt: timestamp,
    },
    {
      id: uid("inv"),
      workspaceOwnerId: userId,
      name: "Koperasi Aisyiyah Grabag",
      whatsapp: "081390120045",
      address: "Kompleks PCM Muhammadiyah Grabag",
      notes: "Titip jual kebutuhan harian untuk kanal amal usaha.",
      isActive: 1,
      createdAt: timestamp,
      updatedAt: timestamp,
    },
  ...
  await db.insert(investors).values(investorRows);

  const miInstan = productBySeedId.get("prd_mi_goreng");
  const deterjen = productBySeedId.get("prd_sampo_sachet");
  const startDate = dateAtDaysAgo(45, 8);
  ...
  await db.insert(investments).values([
    ...
    {
      id: uid("ivt"),
      investorId: investorRows[3].id,
      workspaceOwnerId: userId,
      type: "barang_titip_jual",
      amount: null,
      profitSharePct: null,
      productId: deterjen?.dbId ?? null,
      unitCount: 30,
      unitCost: 18_000,
      profitSharePerUnitPct: 12,
      startDate,
      endDate: null,
      isActive: 1,
      createdAt: timestamp,
  ...
  const reportMonths = [1, 2].map((monthsAgo) => {
    const periodStart = monthStartMonthsAgo(monthsAgo);
    const omzet = monthsAgo === 1 ? 18_750_000 : 16_420_000;
    const labaKotor = monthsAgo === 1 ? 3_260_000 : 2_940_000;
    const bagianPcm = Math.round(labaKotor * 0.3);
    const danaCadangan = Math.round(labaKotor * 0.2);
    const investorPayout = monthsAgo === 1 ? 510_000 : 440_000;
    ...
    return {
      ...
      data: {
        ...
        danaCadangan,
        investorPayout,
        transaksi: monthsAgo === 1 ? 412 : 386,
        notes: "Laporan final demo TokoMu PCM Muhammadiyah Grabag.",
      },
      pdfUrl: monthsAgo === 1 ? "/demo/laporan-pcm-tokomu.pdf" : null,
      status: "final",
      finalizedAt: monthEndMonthsAgo(monthsAgo).toISOString(),
      createdAt: timestamp,
      updatedAt: timestamp,
    };
  });

  await db.insert(monthlyReports).values(reportMonths);

  const restockCandidates = [
    { seedId: "prd_beras_5kg", source: "manual", quantity: 8, unitCost: 68500, note: "Restok beras Mentik Wangi dari grosir Muntilan." },
    { seedId: "prd_mi_goreng", source: "ai_chat", quantity: 24, unitCost: 3200, note: "Restok dicatat dari instruksi chat pengelola." },
    { seedId: "prd_minyak_goreng", source: "ai_ocr", quantity: 12, unitCost: 16500, note: "OCR nota pembelian minyak goreng." },
    { seedId: "prd_susu_uht", source: "manual", quantity: 18, unitCost: 4300, note: "Tambah stok minuman anak sekolah." },
    { seedId: "prd_sabun_cuci", source: "ai_ocr", quantity: 10, unitCost: 8000, note: "OCR nota kebutuhan harian." },
  ] as const;

  await db.insert(restockLogs).values(
    restockCandidates
      .map((item) => {
        const product = productBySeedId.get(item.seedId);
        ...
        return {
          ...
          receiptImageUrl: item.source === "ai_ocr" ? "/demo/nota-restok.jpg" : null,
          ocrRaw:
            item.source === "ai_ocr"
              ? {
                  supplier: "Grosir Grabag",
                  confidence: 0.91,
                  parsedProduct: product.name,
                }
              : null,
          note: item.note,
          createdAt: dateAtDaysAgo(randInt(1, 14), randInt(8, 16)),
        };
      })
  ...
  return {
    ...
    monthlyReports: reportMonths.length,
    restockLogs: restockCandidates.length,
  };
}
...
async function main() {
  ...
  const profileValues = {
    ...
    pcmName: seedState.settings.pcmName,
    pcmChairmanName: seedState.settings.pcmChairmanName,
    pcmAddress: seedState.settings.pcmAddress,
    ...
    profitSharePcmPct: seedState.settings.profitSharePcmPct,
    profitShareReservePct: seedState.settings.profitShareReservePct,
  ...
  const extensionCounts = await seedTokomuExtensions(userId, productList);
  ...
  console.log(`✔ Investor        : ${extensionCounts.investors}`);
  console.log(`✔ Laporan PCM     : ${extensionCounts.monthlyReports}`);
  console.log(`✔ Restock logs    : ${extensionCounts.restockLogs}`);
