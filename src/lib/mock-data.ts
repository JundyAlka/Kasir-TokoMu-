import { AppState, Debt, Expense, Product, Settings, Transaction } from "@/lib/types";

function daysAgo(days: number, hour: number, minute = 0) {
  const date = new Date();
  date.setDate(date.getDate() - days);
  date.setHours(hour, minute, 0, 0);
  return date.toISOString();
}

const products: Product[] = [
  {
    id: "prd_mi_goreng",
    name: "Mi Instan Goreng",
    category: "Makanan",
    buyPrice: 3200,
    sellPrice: 4000,
    stock: 86,
    minimumStock: 24,
    description: "Produk putaran cepat untuk belanja harian warga.",
  },
  {
    id: "prd_air_mineral",
    name: "Air Mineral 600ml",
    category: "Minuman",
    buyPrice: 2200,
    sellPrice: 3500,
    stock: 48,
    minimumStock: 18,
    description: "Stok dingin untuk pembeli dari sekolah dan masjid sekitar.",
  },
  {
    id: "prd_kopi_sachet",
    name: "Kopi Sachet Kapal Api",
    category: "Minuman",
    buyPrice: 1300,
    sellPrice: 2000,
    stock: 72,
    minimumStock: 25,
    description: "Sering dibeli satuan bersama gula dan roti kering.",
  },
  {
    id: "prd_roti_bakar",
    name: "Roti Kering Lokal",
    category: "Makanan",
    buyPrice: 7000,
    sellPrice: 10000,
    stock: 20,
    minimumStock: 8,
    description: "Titipan produsen rumahan sekitar Grabag.",
  },
  {
    id: "prd_teh_botol",
    name: "Teh Botol 350ml",
    category: "Minuman",
    buyPrice: 3800,
    sellPrice: 5500,
    stock: 36,
    minimumStock: 12,
    description: "Minuman siap saji di rak depan kasir.",
  },
  {
    id: "prd_beras_5kg",
    name: "Beras Mentik Wangi 5kg",
    category: "Sembako",
    buyPrice: 68500,
    sellPrice: 76000,
    stock: 14,
    minimumStock: 5,
    description: "Sembako utama dengan nilai transaksi tinggi.",
  },
  {
    id: "prd_beras_ir64",
    name: "Beras IR64 5kg",
    category: "Sembako",
    buyPrice: 62500,
    sellPrice: 70000,
    stock: 16,
    minimumStock: 6,
    description: "Pilihan beras ekonomis untuk keluarga sekitar PCM.",
  },
  {
    id: "prd_gula_pasir",
    name: "Gula Pasir 1kg",
    category: "Sembako",
    buyPrice: 15500,
    sellPrice: 18000,
    stock: 28,
    minimumStock: 10,
    description: "Sering dibeli bersama kopi dan teh.",
  },
  {
    id: "prd_minyak_goreng",
    name: "Minyak Goreng 1L",
    category: "Sembako",
    buyPrice: 16500,
    sellPrice: 19000,
    stock: 22,
    minimumStock: 8,
    description: "Produk sembako cepat habis untuk dapur keluarga.",
  },
  {
    id: "prd_tepung_terigu",
    name: "Tepung Terigu 1kg",
    category: "Sembako",
    buyPrice: 11500,
    sellPrice: 14000,
    stock: 18,
    minimumStock: 7,
    description: "Pelengkap belanja bahan kue dan gorengan.",
  },
  {
    id: "prd_sabun_cuci",
    name: "Sabun Cuci Piring",
    category: "Kebutuhan Harian",
    buyPrice: 8000,
    sellPrice: 10500,
    stock: 26,
    minimumStock: 8,
    description: "Produk rumah tangga dengan repeat order stabil.",
  },
  {
    id: "prd_susu_uht",
    name: "Susu UHT 200ml",
    category: "Minuman",
    buyPrice: 4300,
    sellPrice: 6500,
    stock: 34,
    minimumStock: 12,
    description: "Sering dibeli untuk anak sekolah.",
  },
  {
    id: "prd_telur_kg",
    name: "Telur Ayam 1kg",
    category: "Sembako",
    buyPrice: 27000,
    sellPrice: 31500,
    stock: 12,
    minimumStock: 4,
    description: "Cepat habis, margin sehat.",
  },
  {
    id: "prd_kerupuk",
    name: "Biskuit Kelapa",
    category: "Makanan",
    buyPrice: 6500,
    sellPrice: 9000,
    stock: 30,
    minimumStock: 10,
    description: "Camilan kering untuk keluarga dan tamu pengajian.",
  },
  {
    id: "prd_pasta_gigi",
    name: "Pasta Gigi 75g",
    category: "Kebutuhan Harian",
    buyPrice: 9000,
    sellPrice: 12500,
    stock: 19,
    minimumStock: 6,
    description: "Repeat order bulanan rumah tangga.",
  },
  {
    id: "prd_chiki",
    name: "Snack Anak Sekolah",
    category: "Makanan",
    buyPrice: 2300,
    sellPrice: 3500,
    stock: 58,
    minimumStock: 18,
    description: "Favorit anak-anak setelah pulang sekolah.",
  },
  {
    id: "prd_indomie_rebus",
    name: "Mi Instan Kuah Soto",
    category: "Makanan",
    buyPrice: 3200,
    sellPrice: 4000,
    stock: 64,
    minimumStock: 20,
    description: "Variasi mi instan kuah.",
  },
  {
    id: "prd_sampo_sachet",
    name: "Deterjen Bubuk 800g",
    category: "Kebutuhan Harian",
    buyPrice: 16500,
    sellPrice: 19500,
    stock: 17,
    minimumStock: 7,
    description: "Kebutuhan cuci rumah tangga dengan rotasi mingguan.",
  },
  {
    id: "prd_garam",
    name: "Garam 250g",
    category: "Sembako",
    buyPrice: 3200,
    sellPrice: 5000,
    stock: 24,
    minimumStock: 8,
    description: "Pelengkap dapur wajib.",
  },
  {
    id: "prd_sabun_mandi",
    name: "Sabun Mandi Batang",
    category: "Kebutuhan Harian",
    buyPrice: 4200,
    sellPrice: 6500,
    stock: 32,
    minimumStock: 10,
    description: "Kebutuhan harian dengan permintaan stabil.",
  },
  {
    id: "prd_tisu_wajah",
    name: "Tisu Wajah",
    category: "Kebutuhan Harian",
    buyPrice: 9000,
    sellPrice: 12500,
    stock: 15,
    minimumStock: 6,
    description: "Produk pelengkap di dekat kasir.",
  },
];

const transactions: Transaction[] = [
  {
    id: "trx_001",
    paymentMethod: "QRIS",
    total: 24000,
    paidAmount: 24000,
    changeAmount: 0,
    createdAt: daysAgo(0, 8, 15),
    items: [
      { productId: "prd_mi_goreng", productName: "Mi Instan Goreng", quantity: 2, unitPrice: 4000, costPrice: 3000 },
      { productId: "prd_air_mineral", productName: "Air Mineral 600ml", quantity: 2, unitPrice: 4000, costPrice: 2500 },
      { productId: "prd_kopi_sachet", productName: "Kopi Sachet", quantity: 4, unitPrice: 2000, costPrice: 1200 },
    ],
  },
  {
    id: "trx_002",
    paymentMethod: "Tunai",
    total: 36500,
    paidAmount: 40000,
    changeAmount: 3500,
    createdAt: daysAgo(0, 11, 40),
    items: [
      { productId: "prd_roti_bakar", productName: "Roti Bakar", quantity: 2, unitPrice: 10000, costPrice: 6500 },
      { productId: "prd_teh_botol", productName: "Teh Botol", quantity: 1, unitPrice: 6000, costPrice: 4200 },
      { productId: "prd_susu_uht", productName: "Susu UHT", quantity: 1, unitPrice: 6500, costPrice: 4500 },
      { productId: "prd_kopi_sachet", productName: "Kopi Sachet", quantity: 2, unitPrice: 2000, costPrice: 1200 },
    ],
  },
  {
    id: "trx_003",
    paymentMethod: "Transfer",
    total: 91500,
    paidAmount: 91500,
    changeAmount: 0,
    createdAt: daysAgo(1, 17, 5),
    items: [
      { productId: "prd_beras_5kg", productName: "Beras 5kg", quantity: 1, unitPrice: 72000, costPrice: 64500 },
      { productId: "prd_gula_pasir", productName: "Gula Pasir 1kg", quantity: 1, unitPrice: 17000, costPrice: 14500 },
      { productId: "prd_kopi_sachet", productName: "Kopi Sachet", quantity: 1, unitPrice: 2000, costPrice: 1200 },
    ],
  },
  {
    id: "trx_004",
    paymentMethod: "Tunai",
    total: 44500,
    paidAmount: 50000,
    changeAmount: 5500,
    createdAt: daysAgo(2, 9, 25),
    items: [
      { productId: "prd_minyak_goreng", productName: "Minyak Goreng 1L", quantity: 1, unitPrice: 19500, costPrice: 17000 },
      { productId: "prd_sabun_cuci", productName: "Sabun Cuci", quantity: 1, unitPrice: 11000, costPrice: 8500 },
      { productId: "prd_air_mineral", productName: "Air Mineral 600ml", quantity: 2, unitPrice: 4000, costPrice: 2500 },
      { productId: "prd_mi_goreng", productName: "Mi Instan Goreng", quantity: 1, unitPrice: 4000, costPrice: 3000 },
      { productId: "prd_kopi_sachet", productName: "Kopi Sachet", quantity: 1, unitPrice: 2000, costPrice: 1200 },
    ],
  },
  {
    id: "trx_005",
    paymentMethod: "QRIS",
    total: 32000,
    paidAmount: 32000,
    changeAmount: 0,
    createdAt: daysAgo(5, 13, 10),
    items: [
      { productId: "prd_roti_bakar", productName: "Roti Bakar", quantity: 1, unitPrice: 10000, costPrice: 6500 },
      { productId: "prd_teh_botol", productName: "Teh Botol", quantity: 2, unitPrice: 6000, costPrice: 4200 },
      { productId: "prd_susu_uht", productName: "Susu UHT", quantity: 2, unitPrice: 6500, costPrice: 4500 },
    ],
  },
  {
    id: "trx_006",
    paymentMethod: "Tunai",
    total: 48000,
    paidAmount: 50000,
    changeAmount: 2000,
    createdAt: daysAgo(9, 10, 30),
    items: [
      { productId: "prd_mi_goreng", productName: "Mi Instan Goreng", quantity: 3, unitPrice: 4000, costPrice: 3000 },
      { productId: "prd_sabun_cuci", productName: "Sabun Cuci", quantity: 2, unitPrice: 11000, costPrice: 8500 },
      { productId: "prd_air_mineral", productName: "Air Mineral 600ml", quantity: 2, unitPrice: 4000, costPrice: 2500 },
      { productId: "prd_kopi_sachet", productName: "Kopi Sachet", quantity: 3, unitPrice: 2000, costPrice: 1200 },
    ],
  },
];

const debts: Debt[] = [
  {
    id: "debt_001",
    borrowerName: "Pak Darto",
    whatsapp: "081234567890",
    amount: 85000,
    paidAmount: 0,
    remainingAmount: 85000,
    status: "aktif",
    createdAt: daysAgo(4, 9),
    dueDate: daysAgo(-1, 18),
    isPaid: false,
    lastReminderAt: daysAgo(1, 10, 30),
  },
  {
    id: "debt_002",
    borrowerName: "Bu Rini",
    whatsapp: "081298765432",
    amount: 42000,
    paidAmount: 0,
    remainingAmount: 42000,
    status: "lewat_tempo",
    createdAt: daysAgo(2, 15, 10),
    dueDate: daysAgo(3, 18),
    isPaid: false,
  },
  {
    id: "debt_003",
    borrowerName: "Mas Ari",
    whatsapp: "081355599988",
    amount: 150000,
    paidAmount: 150000,
    remainingAmount: 0,
    status: "lunas",
    createdAt: daysAgo(11, 8),
    dueDate: daysAgo(4, 18),
    isPaid: true,
    lastReminderAt: daysAgo(6, 8, 40),
  },
  {
    id: "debt_004",
    borrowerName: "Toko Sari",
    whatsapp: "081377744411",
    amount: 230000,
    paidAmount: 0,
    remainingAmount: 230000,
    status: "aktif",
    createdAt: daysAgo(1, 12, 20),
    dueDate: daysAgo(-5, 16),
    isPaid: false,
  },
];

const expenses: Expense[] = [
  { id: "exp_001", title: "Belanja gas LPG", amount: 22000, createdAt: daysAgo(0, 7, 10), category: "Operasional" },
  { id: "exp_002", title: "Belanja kulkas minuman", amount: 45000, createdAt: daysAgo(1, 9, 40), category: "Utilitas" },
  { id: "exp_003", title: "Top up stok kopi", amount: 68000, createdAt: daysAgo(2, 14, 10), category: "Belanja" },
  { id: "exp_004", title: "Bayar listrik kios", amount: 125000, createdAt: daysAgo(6, 11, 0), category: "Utilitas" },
  { id: "exp_005", title: "Belanja roti dan susu", amount: 91000, createdAt: daysAgo(10, 8, 15), category: "Belanja" },
];

const settings: Settings = {
  storeName: "TokoMu",
  storeTagline: "Toko Amal Usaha PCM Muhammadiyah Grabag",
  storeAddress: "Grabag, Purworejo",
  pcmName: "PCM Muhammadiyah Grabag",
  pcmChairmanName: "[Nama Ketua PCM]",
  pcmAddress: "Grabag, Purworejo",
  ownerName: "Pimpinan TokoMu",
  ownerWhatsapp: "081277788899",
  city: "Purworejo",
  businessNotes: "Demo SUS testing untuk toko amal usaha PCM dengan fokus sembako, kebutuhan harian, dan barang titip jual.",
  stockAlertThreshold: 8,
  profitSharePcmPct: 30,
  profitShareReservePct: 20,
  enabledPayments: ["Tunai", "QRIS", "Transfer"],
};

export const seedState: AppState = {
  products,
  cart: [],
  transactions,
  debts,
  expenses,
  paymentMethod: "Tunai",
  settings,
};
